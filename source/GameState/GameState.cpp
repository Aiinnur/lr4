...
// Инициализирует базовые классы IWindowKeeper и IState с параметрами
...
// Метод для установки нового контекста игры
void GameState::set_context(GameContext&& context) {
    //Устанавливает новый контекст, передавая его в менедженр
    m_context_manager.set_context(std::move(context));

    // Сохраняет текущий контекст в истории
...
bool GameState::do_step() {
    ...
    if (m_context_manager.get_current_context().state == GameContext::INGAME && !m_do_not_update)
        update();
    render();
    return true;
}
...
void GameState::update() {
    ...
    auto find_food = [&pacman](const std::unique_ptr<IStaticEntity>&el) { return &pacman.get_location() == &el->get_location(); };
...
void GameState::render() {
    ...
    // static_objects
    for (auto& static_object : m_context_manager.get_current_context().static_objects)
        static_object->draw_into(m_window);

    // dynamic_objects
    for (auto& dynamic_object : m_context_manager.get_current_context().dynamic_objects)
...
void GameState::process_key_pressed(sf::Keyboard::Key key) {
    if (m_context_manager.get_current_context().state == GameContext::INGAME) {
        auto& new_pacman = m_context_manager.get_current_context().pacman;
        switch (key) {
